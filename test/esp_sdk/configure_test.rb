...
class ConfigureTest < ActiveSupport::TestCase
  context 'Configure' do
    setup do
      ...
      @config = ESP::Configure.new(email: 'test@test.com', token: 'token')
    end
    ...
    context '#initialize' do
      ...
      should 'should setup the token and token and expires at' do
        ESP::Configure.any_instance.unstub(:token_setup)
        ...
        config = ESP::Configure.new(email: 'test@test.com', password: 'password1234')
    ...
    context 'url' do
      should 'return domain with api/version/end_point' do
        @config = ESP::Configure.new(email: 'test@test.com', token: 'token', domain: 'https://test.domain.com', version: 'v4')
    ...
    context '#domain' do
      should 'return custom URI when the URI is passed in' do
        @config = ESP::Configure.new(email: 'test@test.com', token: 'token', domain: 'https://test.domain.com')
      ...
      should 'return the release URI when the environment is release' do
        ESP.expects(:production?).returns(false)
      ...
      should 'return the development URI when the environment is not release or production' do
        ESP.expects(:production?).returns(false)
        ESP.expects(:release?).returns(false)
