      ...
      context "with ESP::Alert" do
        context '#parse_pagination_links' do
          should 'not set the previous page or next page or last page when there is only 1 page' do
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 2, page: { number: 1, size: 20 }))
          ...
          should 'not set the previous page when on the first page' do
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 2, page: { number: 1, size: 1 }))
          ...
          should 'not set the next or last page page when on the last page' do
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 2, page: { number: 2, size: 1 }))
          ...
          should 'set the next, last and previous page page when not on the first or last page' do
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 3, page: { number: 2, size: 1 }))
          ...
          should 'not set page size on previous link if on the last page' do
            ...
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 3, page: { number: 2, size: 2 }))
        ...
        context '#first_page' do
          should 'call the api with the original url and params and the page number 1 param' do
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 3, page: { number: 2, size: 1 })).then.to_return(body: json_list(:alert, 3, page: { number: 1, size: 1 }))
          ...
          should 'not call the api and return self if already on the first page' do
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 3, page: { number: 1, size: 1 }))
            alerts = report.alerts
        ...
        context '#next_page' do
          should 'call the api with the original url and original params and the next page number param' do
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 3, page: { number: 2, size: 1 })).then.to_return(body: json_list(:alert, 3, page: { number: 3, size: 1 }))
            alerts = report.alerts(status: 'pass')
          ...
          should 'not call the api and return self if already on the last page' do
            report = build(:report)
            stub_request(:put, %r{reports/#{report.id}/alerts.json_api*}).to_return(body: json_list(:alert, 3, page: { number: 3, size: 1 }))
            alerts = report.alerts
